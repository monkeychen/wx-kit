...
![Status](https://img.shields.io/badge/v0.8.2-released-success.svg)
...
- **抓得到全部消息类型**:此前用的列表接口只返回「图文素材」——某号 7 月中旬后发的文字消息与视频消息**一篇都看不见**(实测最近 10 篇里 9 篇抓不到)。换接口后齐全;**订阅检查同链路,此前一直静默漏检**,修好后回溯补回。
- **含视频的文章能下了**:视频是内容不是格式,有就一并下载(自动取最高清档),`html` 在阅读器里直接可播。单个可达上百 MB,设置里可关、CLI 用 `--no-video`。
- **订阅检查会说话**:自动下载模式下此前全程静默(页面毫无变化),现在逐号就地反馈 + 进度 + 「去看看」直达文库,并常驻显示当前策略。
- **更新检查**:启动静默查,有新版只在「设置」点一个小圆点(不弹窗);**按你的安装方式给升级动作**——brew 装的给一条可复制的完整命令。
- **未适配的新消息类型不再静默产出垃圾**:走兜底但会在 `warnings` 里报出来(此前视频消息页曾把 21.8 万字符内联 JS 当正文)。
...
去 [Releases](../../releases) 选平台对应包(最新 v0.8.2:`wx-kit-0.8.2-arm64.dmg`(Apple Silicon) /
`wx-kit-0.8.2.dmg`(Intel) / `wx-kit Setup 0.8.2.exe`(Windows))。当前**未签名/未公证**,首次打开需手动放行:
...
**v0.1.0 – v0.8.2 均已发布**(最新 **v0.8.2**:不让用户猜)。各里程碑均合入 main,端到端在真实微信公众号后台验证通过:
