...
![Status](https://img.shields.io/badge/v0.8.3-released-success.svg)
...
- **下载**任意微信文章为封面 / Markdown / 网页 / PDF / 元信息 5 种格式,**文中视频一并下载**;
- **各类消息都能抓**:普通图文、文字消息、视频消息、图文消息(小绿书);
- **批量爬取**某个公众号的历史文章(按数量或日期范围);
- **库内阅读**已下载文章;
- **同二进制**带 CLI(`npx electron . download ...`),面向 AI agent 自动化调用。

**最新版本 v0.8.3(打不开的文章不再算「下载失败」)**:

- **失败原因说人话**:作者发布失败/违规下架的文章,过去报笼统的 `no title parsed`,现在直接说「该文章审核未通过,读者不可见」。
- **两类彻底分开**:批量汇总里 `失败` 只报**真故障**(网络、解析等),读者本就打不开的单独说明——不会再让人以为工具坏了。
- **列表过滤补齐**:除 `is_deleted` 外补上 `checking`(审核期间)与 `ban_flag`(封禁),这类文章不再进下载队列、也不再被订阅检查当成新文章。
- **`--count N` 按能下的篇数算**:跳过不可见的继续往前补,且只在结果确实不及预期时才解释。

上一版 v0.8.2(不让用户猜):抓得到全部消息类型(旧接口只返回图文素材,订阅一直静默漏检)、含视频文章自动下载、订阅检查逐号反馈、更新检查 + 按渠道引导升级。
...
去 [Releases](../../releases) 选平台对应包(最新 v0.8.3:`wx-kit-0.8.3-arm64.dmg`(Apple Silicon) /
`wx-kit-0.8.3.dmg`(Intel) / `wx-kit Setup 0.8.3.exe`(Windows))。当前**未签名/未公证**,首次打开需手动放行:
...
**v0.1.0 – v0.8.3 均已发布**(最新 **v0.8.3**:打不开的文章不再算「下载失败」)。各里程碑均合入 main,端到端在真实微信公众号后台验证通过:
...
**v0.8.3 · 补丁(2026-07-27)**
- ✅ M38 — 「读者打不开」与「下载失败」分开:错误页认出具体原因、汇总分两类计数、列表补齐 `checking`/`ban_flag` 过滤

**v0.8.2 · 不让用户猜(2026-07-26)**
- ✅ M34 — 订阅检查反馈:逐号明细 + 自动下载进度 + 「去看看」+ 策略常驻
- ✅ M35 — 含视频文章下载:视频作为内容自动下载(最高清档),md/html 引用改写
- ✅ M36 — 列表接口修正 + 消息类型一等公民:抓得到文字/视频消息,未知类型进 `warnings`
- ✅ M37 — 更新检查 + 按渠道引导升级(不做静默自更新)
