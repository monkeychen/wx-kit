...
命中频控立即持久熔断且不自动重试;设置页可安全重新登录或彻底退出且不删除用户设置。
当前只完成网络封锁下的离线验收,尚未做真实微信联调。
...
- ✅ M47 — 设置页提供重新登录/彻底退出;只清公众号 session、专用分区与缓存,保留设置、订阅、文库和频控状态
...
- ✅ M40 — 订阅新文章展开可见、勾选下载/忽略 + 文库类型标识修正与解析告警可见;顺带修掉订阅下载丢文章主键致重复下载、留存的待处理被下次检查冲掉、失败的文章被静默清出列表
- ✅ M41 — CLI `subscription digest --date`:查订阅号某一天发了什么(不下载、不写库、不推水位),`downloaded` 让 agent 分流
- ✅ M42 — `wx-kit-compose` 跟上 CLI:上游 `digest` 起步、下游可选 `site sync`、补素材质量信号;命令细节改为指向 `wx-kit-skill`(抄来的不会跟着源头变)
