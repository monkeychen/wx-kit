...
description: |
  ...
  基于本地文库生成可追溯选题（v0.12.0 起；GUI 与 CLI 均需用户自备兼容模型 API Key）、同步 Astro 站点——stdout 输出纯 JSON，面向 agent 自动化。
...
本文以 v0.12.0 已发布能力为基线。旧版安装包（v0.11.3 及更早）没有“选题”导航或 `topics` 命令，先 `wx-kit --version` 再下结论，不把版本差异当成安装故障。
...
| 从本地文库生成选题 | GUI“选题” / `wx-kit topics analyze --range 24h` |
| 从已保存候选生成简报 | GUI 点候选后“生成选题简报” / `wx-kit topics brief --run <runId> --topic <topicId>` |
