...
这是 v0.12.0 起的正式能力。旧版（v0.11.3 及更早）没有该命令，属版本差异而非安装故障。

M70 起增加 GUI“选题”导航：先在“设置 → AI 模型”完成配置，再选素材分析。M78 起 GUI 只提供「输入素材」人工选篇（时间范围/自定义日期仅 CLI 保留）；选稿弹层筛选为主、搜索为辅（M77），行内「看一眼」看摘要或正文开头、「查看原文」跳阅读器，返回选稿时弹层与已选自动恢复。历史入口可回看与删除过往 run（删除分「仅移除记录」与「连同本地文件」两档）。GUI 默认不选中任何候选；点选后才展开理由、材料依据和起笔结构。反馈、切换页签和生成简报都不会再调模型。分析期间页面显示模型实时输出（正文与思考分段、自动滚动），运行中可随时取消，切走再回来进度与已生成内容不丢；没有总超时限制，长时间无响应会明确报空闲超时。
