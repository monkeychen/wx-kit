...
从 v0.10.1 / M55 源码起，“某天发表的订阅文章”先用 `subscription digest` 从本地文库按发表日期取料，
默认零网络、无需登录。仅用户明确要求刷新下载今天内容时才加下载选项；历史日期不能借此回补。
参数与日期限制以 `wx-kit-skill` 为准；检查未知发表时间告警，不把本地收集清单当完整发布史。
