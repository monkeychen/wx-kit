...
> **状态：实现与本地验收完成，待正式发布**（2026-08-08）。
>
> 最新公开版本仍为 v0.8.5。v0.8.6、v0.8.7 从未发布，相关有效成果和需求去向统一由本 PRD 承接。
...
- 本文件是当前开发版本唯一验收契约。
- `package.json` 和安装包版本只在正式发版步骤中从 0.8.5 升到 0.9.0。
...
- v0.9.0 发布仍须单独执行版本 bump、三平台出包、打包应用验证、GitHub Release 和 brew tap 核实。
...
- **M51 · 当前产品说明与发布准备**：已完成；有效页面截图、README/Skill/发布草案和打包应用均已验收。
...
- [x] README、四张真实数据截图、Skill、ROADMAP、发布说明草案与当前产品边界一致。
...
- [ ] 正式发版时三平台资产、GitHub Release 与 brew tap 全部发布并逐项核实。
...
审计无 `/cgi-bin/`。macOS arm64/x64 应用和 DMG 构建完成；正式发版时再升根包版本并补 Windows 包。
...
- 未明确执行“发布 v0.9.0”前，不 push、不打 tag、不创建 Release、不更新 brew。
