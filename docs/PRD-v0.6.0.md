...
> **状态:已实现并发布**(2026-07-19 随 v0.6.0 发版;逐条验收见 §2 各 R)。
> 实现计划(启动后):`docs/plans/`;状态/进度:`ROADMAP.md`。

## 1. 一句话定义

**让 agent 不碰鼠标就能用起 wx-kit**——一条命令安装(npm/brew)、headless 环境可迁移登录态、帮助与专属 skill 让 agent 自学自用;顺带把文库默认排序、检查日志入口两处日常体验补顺。

两条主线:
- **Agent 自动化闭环(主)**:R4 装得上 → R5 登得上 → R3 读得懂 → R6 用得起来。
- **日常体验(辅)**:R1 文库默认发布时间降序 + 排序记忆;R2 检查日志入口。
...
**验收(草)**:
- [x] 全新安装/未动过排序时,进文库卡片与列表视图均按发布时间降序,最新发表在最前(e2e 断言默认即最新在前)。
- [x] 无发布时间的文章恒在末尾(升降序皆然;既有单测锁定)。
- [x] 切换排序键/方向后持久化,跨导航/重开保持(e2e 跨导航断言;settings 单测)。
...
**验收(草)**:
- [x] 设置页订阅区可见「打开检查日志」入口(e2e 断言;复用既有 openLog IPC)。
- [x] 日志文件不存在时不报错(ipc 打开前 append 空串确保文件存在,既有行为)。
- [x] 订阅页原入口行为不变,文案统一「📄 打开检查日志」提升可发现性。
...
**验收(草)**:
- [x] `wx-kit -h` 覆盖双模式/命令组子命令/输出契约/示例/库位置(单测断言关键串;打包态目检)。
- [x] 各子命令 `-h` 保持 commander 默认行为(既有 CLI 测试全过)。
- [x] `-h` 走 stdout、退出码 0(单测断言)。
...
**验收(草)**:
- [x] mac 隔离环境 brew 安装 → `--version` + `download`(含 pdf)全通。⚠️ 验收原文「无手动放行」不成立:Homebrew 5 移除 `--no-quarantine`,且实测 quarantine 连 CLI 都拦——修正为「安装后一条 `xattr -cr` 即可用」,已写死进 cask caveats/README/skill。
- [x] npm tarball 隔离全局装 → `--version` + 真实下载全通(mac)。Linux 未实测(无环境),包声明 os: darwin/linux,Linux 实测待有环境补。
- [x] skill 文档含检测→安装样例(SKILL.md 第一步)。
- [x] 发版规约⑦⑧已入 AGENTS.md(tap 刷新以已发布资产计算 sha256;真实安装验证步骤)。
...
**验收(草)**:
- [x] 真机 export → 隔离 HOME 实例 import → valid:true → download 可用(M28 端到端亦覆盖)。
- [x] 导入非法文件报 CLI_ERROR 退出码 2、既有 session 不被触碰(单测 + CLI 契约测试)。
- [x] 过期 session 导入如实 `valid:false`(mock 探测单测);网络失败 `valid:null` 不误判。
- [x] 导出 0600(真机核验 -rw-------);无 session NO_SESSION 退出码 1(契约测试)。
...
**验收(草)**:
- [x] 全新 agent 四步端到端(隔离 npm prefix + 隔离 HOME):装 → session import(valid:true)→ download → library list,零人工(2026-07-19)。
- [x] skill 覆盖全部 CLI 原子能力,样例逐条实测(纠出 library export 输出形态的文档错误)。
- [x] 与 wx-kit-compose 无重复,agent/README 互相指路。

## 3. 里程碑拆分

| 里程碑 | 范围 | 依赖 | 状态 |
|--------|------|------|------|
| **M25** | 日常体验:R1 排序默认值+跨会话记忆 + R2 检查日志入口(设置页新增/订阅页提升可发现性)+ R3 CLI 帮助完善 | 无 | ✅ 2026-07-19 |
| **M26** | 安装通道:R4 brew tap + npm 包 + 发版规约扩展(tap sha256 / npm publish 脚本化) | 无 | ✅ 2026-07-19 |
| **M27** | headless 登录态:R5 `session export`/`import`(结构校验 + 真探测 + 0600 + 安全提示) | 无 | ✅ 2026-07-19 |
| **M28** | agent skill:R6 `agent/wx-kit-skill/`(检测→安装→登录态→原子能力→组合范例) | M26/M27(引用其命令) | ✅ 2026-07-19 |

> 拆分理由:M25 三条互不依赖但都是小改动,合并为一个「体验杂项」里程碑;M26/M27 各自独立可并行;M28 是收口,其安装/登录段落引用 M26/M27 的成果,必须最后做。发版(R7 性质的常规发版步骤)随最后一个里程碑走发版规约,不单列。

## 4. 非目标

- **Windows 安装通道(winget/scoop)与 Windows CLI stdout 正解**——win 的 GUI 子系统 stdout 问题未解,agent 场景优先 mac/Linux;仍留 ROADMAP 候选。
- **GUI 的 session 导入/导出入口**——headless 专属场景,GUI 用户直接扫码(R5 已述)。
- **mac 签名/公证**——brew `--no-quarantine` 已绕开手动放行;签名需付费开发者账号,单独议。
- **npm 包的自动更新机制**——npm/brew 本身的 upgrade 命令即更新通道,不做应用内更新。
- **多 session/多公众号后台账号**——延续既往非目标。
