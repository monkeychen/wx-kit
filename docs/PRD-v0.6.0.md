...
> **状态:需求已收齐**(R1–R6,2026-07-19 安哥确认收集完成),**待安哥宣布启动**后写实现计划、开工。
...
| 里程碑 | 范围 | 依赖 | 状态 |
|--------|------|------|------|
| **M25** | 日常体验:R1 排序默认值+跨会话记忆 + R2 检查日志入口(设置页新增/订阅页提升可发现性)+ R3 CLI 帮助完善 | 无 | ⏸ 待启动 |
| **M26** | 安装通道:R4 brew tap + npm 包 + 发版规约扩展(tap sha256 / npm publish 脚本化) | 无 | ⏸ 待启动 |
| **M27** | headless 登录态:R5 `session export`/`import`(结构校验 + 真探测 + 0600 + 安全提示) | 无 | ⏸ 待启动 |
| **M28** | agent skill:R6 `agent/wx-kit-skill/`(检测→安装→登录态→原子能力→组合范例) | M26/M27(引用其命令) | ⏸ 待启动 |
