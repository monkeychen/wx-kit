...
- **格式可选**（多选，至少选一项）：
  ...
  - PDF（pdf）
  - 文章元数据（meta，JSON）
- **行为**：所有 URL 进入统一下载队列，逐条处理；实时展示每条的状态（等待 / 下载中 / 成功 / 失败 + 失败原因）。
...
| `library` | 维护 `library.json` 索引，提供列表/查询/删除 | 文件系统 |
...
- [ ] 文章库能列出、筛选、在内置阅读器中打开 md/html，能删除。
