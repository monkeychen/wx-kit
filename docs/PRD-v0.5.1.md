...
- [x] 下载后标题为正文首行截断（≤30 字 + `…`），**不含**字面 `\n`；目录名正常长度、无 `_n_n`。
- [x] `content.md` 正文完整分段（该篇 12 段全在），frontmatter title 干净。
- [x] `index.html` 正文完整分段，`<title>`/`<h1>` 干净。
- [x] GUI 阅读器打开该文章：标题栏一行短标题，正文完整可读（真机验证 2026-07-09，隔离 userData + Playwright 截图）。

**图文消息**（验证 URL：`https://mp.weixin.qq.com/s/2enR9fGb9oQ0edZlplkVxA`）：
- [x] 下载后标题为 og:title（「有海鸥还看什么A股行情...」），正文文字完整分段。
- [x] 图片全部提取并本地化到 `images/`（该篇 7 图全在；不含水印图/分享封面图的重复项），md/html 中图片引用指向本地文件。
- [x] GUI 阅读器打开该文章：文字 + 图片完整呈现，7 图 `naturalWidth > 0`（真机验证 2026-07-09）。

**回归与通用**：
- [x] 标准图文文章（既有 fixture `sample-article.html`）解析结果不变（标题/正文/图片/时间/公众号）。
- [x] 已删除文章错误页仍判定为无效（「标题为空」路径不被新分支绕过）。
- [x] 两种新类型的解析纯逻辑 TDD 覆盖（真实页面裁剪 fixture），含 JS 转义还原、图片锚定排除水印/封面、标题截断边界。
...
- [x] main 打 annotated tag `v0.5.1` + GitHub Release 三平台包（2026-07-09 已发,标 Latest）。

## 5. 里程碑拆分

| 里程碑 | 范围 | 状态 |
|--------|------|------|
| **M19** | 文字消息 + 图文消息解析支持（R1） | ✅ 已完成（2026-07-09） |
