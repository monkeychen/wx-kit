...
- [ ] **视频消息页(10002)的正文不再是内联 JavaScript**:content.md 是描述文字段落,不含 `<script`(现状 21.8 万字符垃圾)。此条**不依赖是否选 video 格式**。
- [ ] 择档按分辨率:测试文章取到 `f10002`(1572×1080/133.5MB),而非 format_id 最大的 `f10104`(480×328)。
- [ ] 选 `video` 格式下载测试文章:`videos/video-1.mp4` 落盘,是可播放的完整 mp4(非 206 半截),体积与 `filesize` 对上。
