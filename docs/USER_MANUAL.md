...
>
> 截图分两类：*GUI 截图*来自 v0.12.0 真实应用（`docs/screenshots/`）；*插画*为辅助说明使用 AI 生成（`docs/screenshots/illustrations/`），风格与 GUI 截图配色保持一致。macOS 排版；Windows 控件位置一致。

---

## 0. 这个 app 是干嘛的

**微信百宝箱 (wx-kit)** 是个桌面应用，把微信公众号的文章下载到本地文库，在 app 内阅读、搜索、整理，并可选地发布为个人站点。同时它也能下载「墨问」笔记（独立笔记平台，作者订阅 + 关键词搜索），并在 v0.12.0 起基于本地素材生成可解释的**选题候选**（需自配 AI 模型服务）。
...
打开 wx-kit，顶部从左到右：**下载**（粘 URL 拉文章）、**订阅**（后台自动拉新文章）、**文库**（本地阅读与搜索）、**选题**（从素材到候选，v0.12.0 起）、**设置**（偏好与策略）。阅读器在文库或选题里点开单篇时进入。
...
### 7.8 AI 模型（v0.12.0 起，选题功能的前置）

选题功能需要一个 AI 模型服务。到「设置 → AI 与工具 → AI 模型」：

- **选厂商**：智谱 / 千问 / DeepSeek / Kimi / MiniMax / OpenAI / Google，或自定义（手填 base URL）；
- **填 API Key**：在你所选厂商的官网申请。Key 用系统加密保存；系统加密不可用时只保存在本次会话（重启需重填，明文绝不落盘）；
- **选模型**：可从候选下拉选，也可自由输入；保存前点「测试连接」验证；
- 推理偏好（模型「思考」深浅）按厂商能力显示，不动它就用默认。

隐私边界一句话：**你选中分析的文章正文会发送到上述你配置的服务**；wx-kit 本身不内置任何模型服务，也不经手你的 Key 之外的数据。不配置 AI 模型不影响下载、阅读、订阅等其它功能。

---

## 8. 选题：今天写什么（v0.12.0 起）

素材攒了一堆不知道先写哪篇？选题页帮你把「翻素材」变成「拿候选」。

![选题页](screenshots/topics-picked.png)

### 怎么用（三步）

1. **选素材**：点「输入素材」→ 在弹层里从文库挑文章。左上先按时间收窄（默认近 30 天，可切全部）、按来源过滤；列表里每篇有「看一眼」（快速看摘要或正文开头）和「查看原文」（跳阅读器细读，返回后勾选还在）。选好点「用这 N 篇分析」。
2. **看候选**：点「寻找选题」，分析过程实时流式显示，完成后出**最多三张候选卡**——每张写明题目问题、写作角度、读者价值、推荐理由。三张平等展示，**选不选、写不写由你**。
3. **下判断**：点开候选看「为什么值得写 / 材料依据 / 怎么下笔」。材料依据里的摘录能在原文中逐字找到（但记住：摘录存在 ≠ 事实已验证）。顺手记个反馈（暂不写 / 保存观察 / 已经写过），或一键生成 Markdown 简报去别处细看。

### 历史与删除

顶部「历史选题」列出每次分析（时间倒序，含状态与候选标题）。回看历史现场时反馈、简报都还能用。删除时两档分明：**仅从列表移除**（本地文件保留）和**删除记录和本地文件**（物理清除，不可恢复）。

### 三条诚实话

- 一次最多 30 篇文章 / 12 万字；素材不足或模型输出不可信时，会明确告诉你失败原因，**不会编三张卡凑数**；
- 「传播效果：未验证」是固定标注——工具不预测流量；
- 文章正文会发给你配置的 AI 服务（见 7.8），介意的话别选敏感文章。
...
# 选题侧（v0.12.0 起，需已配置 AI 模型；环境变量带 Key）
export WXKIT_AI_API_KEY='你的Key'
wx-kit topics analyze --range 24h
wx-kit topics analyze --article <文章id,文章id>
wx-kit topics brief --run <runId> --topic <topicId>
```
