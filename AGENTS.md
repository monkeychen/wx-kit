...
- **私有文章列表能力已通过微信读书后端复活（v0.10.0）**：M49 因 MP 后台失效而退场的订阅与批量能力，已在 v0.10.0 经由微信读书 Web API 接口（`/api/mp/cover`）重新实现。**但旧 MP 后台接口仍不得接回网络。**
  - **网络栈指纹分级（2026-08-27 实测，勿再凿）**：微信读书按「请求发起端的网络栈」给会话分级——真·谷歌 Chrome 的请求能拿到全量列表（`web/mp/articles`，每页 20 可翻页）；**Electron 自带栈和 Node undici 即使 Cookie 正确也恒被列表接口以 `-2041` 拒绝**（cover/shelf/bookInfo 不受限）。同一份降级 Cookie 在系统 Chrome 里立即可列，证明是按请求的网络指纹判定而非登录时烙死。故 v0.10.0 内「按公众号批量下载历史」在本架构内无解：生产实现统一走 Electron 栈（`weread-net.ts`），列表失败自动回退 `cover` 最新一篇；增量订阅不受影响。破局只能靠引入外部真浏览器依赖（违背纯 Node 决策）或官方放开，需要时单独立项。
  - **重要降级**：新接口每次仅返回公众号**最新一篇**文章，无法全量回补历史，仅适用于增量订阅。
