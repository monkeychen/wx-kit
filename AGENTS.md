...
- **文章列表用 `cgi-bin/appmsg?type=9`,不用 `appmsgpublish`**(M44 实录,2026-08 安哥定):换用 `appmsgpublish` 后约一天即触发账号级频控(`200013`),探针证实**两端口同 ret、与端点无关**(换接口解不了频控);但 `appmsgpublish` 每页 20 组、返回数据多、更耗频控配额,解封后更易再触发,而 `appmsg` 每页实回 ~5、更轻,故权衡换回。**代价**:`appmsg` 只回 `item_show_type=0` 的图文,**文字消息(10)与视频消息(5)不进列表**(单篇仍可经 URL 下载,消息类型在下载阶段从文章 HTML 重读,文库卡片标识不丢)。**与端点解耦、换回时保住的关键机制**:去重主键链(`appmsgid`/`itemidx` 透传,缺了还有长链 `mid`/`idx` 兜底)、解析阶段类型识别、不可见文章下载阶段兜底(`ArticleUnavailableError`;列表不再预过滤 `is_deleted`/`checking`/`ban_flag`,改由下载阶段认出)。**别又来回切**:v0.8.2 R4(M36)为覆盖全消息类型从 `appmsg` 换到 `appmsgpublish`,M44 因频控权衡换回——每次切换都有连锁代价(去重主键语义、不可见过滤),决策与权衡见 `docs/plans/2026-08-02-m44.md`。
