...
- **当前开发:v0.10.1（M54 已完成；M55 方案已确认、待实施）** —— M55 日报以 `library.json.publishTime` 为唯一文章来源；默认（含 today）纯本地查询，仅今天显式加 `--download` 才刷新 cover、下载缺失文章并重读文库。撤销独立发现日志，不按调度时间隐式联网，发布时间缺失保留正文并告警。需求/验收 `docs/PRD-v0.10.1.md`，计划 `docs/plans/2026-08-30-m55-publication-digest.md`。
...
| **M55** 🟡 | v0.10.1 | 方案已确认、待实施：日报按本地文库发表日期查询，默认零网络；仅今天 `--download` 刷新并下载；未知发表时间告警，不新增存储文件 | `plans/2026-08-30-m55-publication-digest.md` |
