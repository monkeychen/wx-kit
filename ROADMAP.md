...
- **当前开发:v0.10.1（M54、M55 已实施，未发版）** —— M55 日报以 `library.json.publishTime` 为唯一文章来源，默认（含 today）纯本地，仅今天显式 `--download` 刷新下载并重读文库；缺失时间保留正文并告警。单测/lint/tsc、双架构构建、GUI fixture e2e、打包本地查询及首次真实 cover 下载已通过。跨进程第二次刷新 HTTP 401 已定位为成功请求后的 Cookie 快照未回写，源码与单测修复完成；需在下一次有效扫码登录后复验真实连续进程。验收详情见 `docs/plans/2026-08-30-m55-publication-digest.md`，需求见 `docs/PRD-v0.10.1.md`。
