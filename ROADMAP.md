...
- **CLI wrapper 临时位置守卫** —— 从构建目录（`release/`）启动 app 后在设置页点「重建命令行入口」，wrapper 会指向构建产物（`process.execPath`），目录被下次打包清掉后 CLI 悬空（2026-09-04 实录，从正式安装启动/重建即自愈）。修复需设置页在临时位置时拦截并提示，待有真实用户场景再做。
