...
| **M36** 🚧 | v0.8.2 | **列表接口修正 + 消息类型一等公民**:`appmsg?type=9`(370 篇/只有图文/最新卡在 7-17)换成 `appmsgpublish`(770 篇/全类型/每页 20);解析改按 `item_show_type` 显式分发,未知类型进 `warnings` 不再静默产出垃圾 | `plans/2026-07-26-m36.md` |
| **M37** 🚧 | v0.8.2 | 更新检查 + 按渠道引导升级:GitHub API 查最新版、自写版本比较、识别 brew/dmg/nsis 渠道给一键动作、启动静默检查 + 不打扰提示 | `plans/2026-07-27-m37.md` |
