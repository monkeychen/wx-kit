...
- **当前开发:v0.10.1（M54 已完成，待发版）** —— `cover` 以稳定 `reviewId` 游标判重，首检仍投递最新一篇一次，旧订阅已下载文章升级后不重报；订阅页可一键下载全部待处理新文章，移除已证伪的列表探测，并显示正文/图片/视频/导出阶段。需求/验收 `docs/PRD-v0.10.1.md`，实现计划 `docs/plans/2026-08-29-m54-subscription-reliability.md`。
